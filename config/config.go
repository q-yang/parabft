...
type Config struct {
	Addrs     map[identity.NodeID]string `json:"address"`      // address for node communication，node.go里面通过socket.NewSocket用到了Addrs
	...
	ChanBufferSize int             `json:"chan_buffer_size"` // buffer size for channels配置文件中定义的缓冲大小，node.go里面有用到
	...
	Pprof          bool            `json:"pprof"`     //作恶策略，有分叉以及沉默攻击
	MaxRound       int             `json:"maxRound"`
	Strategy       string          `json:"strategy"`     //作恶策略，有分叉以及沉默攻击,silence是沉默攻击
